...
/* read & write */
connection.then(() => {
    return swarmdb.put(owner, dbname, tablename, [ {"age":1,"email":"test001@wolk.com"}, {"age":2,"email":"test002@wolk.com"} ]);
})
.then((data) => {
...
.then((data) => {
    ...
    return swarmdb.query(owner, dbname, "select email, age from contacts where email = 'test002@wolk.com'");  
})
.then((data) => {
